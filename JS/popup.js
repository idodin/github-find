$(document).ready(function(){
    ...
    window.open = function(){
        ...
        $("#1-panel").css('display', 'none');
        $("#2-panel").css('display', 'none');
        $("#3-panel").css('display', 'none');
    }
...
chrome.tabs.query({active: true, currentWindow: true, lastFocusedWindow:true}, function(tabs) {
      chrome.tabs.sendMessage(tabs[0].id, {greeting: "hello"}, function(response) {
          chrome.storage.sync.get('repos', function(data) {
              ...
              var name = data.repos.data[0].name.split("/");
              if(data.repos.features.length >= 1) {
                  var topicstr = data.repos.features[0].label;
                  for (var i = 1; i < data.repos.features.length-1; i++) {
